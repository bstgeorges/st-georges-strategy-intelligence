...
The current edition is Sunday 13 September 2026, **Edition 13**: *The gap is not approval. It is proof.* The underlying 13 September package was production-verified by the green `Site release (Cloudflare)` workflow run `34748387631`, for commit `1fcbb89c1d7cabb47f4c82f6d3ba5d418f3927ee`. A same-day editorial correction now updates the Judgement, Brief and featured Committee Question together; its new release run is the publication proof for the corrected copy.
...
## Session memory — 2026-08-29 to 2026-09-13: product, data and release progress

- **Public editions:** Edition 11 was published on 30 August and Edition 13 on 13 September. The release standard remains substance before date: a new date without a renewed shortlist, judgement and linked evidence is a failure, not a refresh. The current 13 September correction is an intentional archive correction, so build it with `SGS_ARCHIVE_CORRECTION=1`, inspect the dated archive diff and keep the corrected snapshot with the release.
- **Editorial product signature:** Use the natural flow **Signal → Judgement → Question → Evidence** across Home, Brief, Committee Questions and LinkedIn, without presenting it as a second branded framework. Keep the reader-facing Judgement in the three beats **What happened / Why it matters / What to do**. It should sound like an editor making one defensible call, not an AI-generated source roll-call or a generic assurance checklist.
- **Judgement distinctness rule:** Before publication, compare the proposed Weekly Judgement and LinkedIn copy with the preceding three editions. Record `judgement.editorialAngle` and `judgement.distinctFromPrevious` in `site/data/current-edition.json`; `release:readiness` requires both. Reject repeated action grammar and the default bundle of owner, map, dependency, test, intervention, exception and closure. Select one concrete decision or failure mode, state the observed behaviour in plain language, and make the final action one specific next move. The detailed evidence pack belongs in the Brief or Committee Questions when needed.
- **Edition 13 editorial decision:** The Mistral parity-harness signal is not another general change-control story. Its distinct decision is whether the firm can prove that a live change still meets the approved baseline. The reader action is to define two or three outcomes that must remain true, test them before the next release, and escalate a failed or unresolved difference. This is the approved model for the corrected 13 September Judgement and related Brief/featured Committee Question.
- **Signals:** The governed direct source estate has expanded beyond 80 active primary feeds across all eight topics; candidates are a private review queue, never publication authority. Materiality matters more than a five-item quota, especially in resilience: retain a verified anchor and record the gap rather than force weak material. Every Top 5 display title is an editorial signal, linked directly to the primary article; literal provider wording stays in `evidence.sourceTitle`. Check direct source dates—sitemap `lastmod` is not a publication date unless separately governed and verified.
- **Signals health and release gates:** Candidate promotion needs an approved dated shortlist, a current promotion summary, verified dates and the `signals:health:verify` report. Temporary link transport outages may use only the documented, reviewed exceptions; HTTP failures cannot be waived. The link-audit and source-date gates are part of the production workflow, not optional clean-up.
- **Regulatory Horizon:** The public Horizon remains withheld pending a deliberate editor/product-owner relaunch decision, even though the private register is now a useful internal operating tool. The private dashboard has been made more reader-ready: it presents confirmed forthcoming dates, direct official links, useful filters and a restrained profile; it omits raw source-coverage and review-queue mechanics from the reader surface. On 13 September the private quality state was 100/100: 10 confirmed open dates, six authorised source states and seven healthy core authorities. This validates the internal evidence process, not public completeness or external applicability.
- **Horizon reliability:** Only explicit primary-document deadline/effective-date language can create a deadline; a publication date cannot become one by inference. Keep scanner material labelled private review and record every manual decision as confirmed, rejected, not applicable or superseded. Preserve known source exceptions honestly (including blocked, failed or degraded sources); do not use a successful scan to imply comprehensive market coverage. The pre-relaunch work remains three distinct-day private scans, repair/govern exceptions, verify genuine future-dated records and then seek explicit relaunch approval.
- **Deep Dives:** `/deep-dives/` is the reader-facing index and the sole Deep Dives navigation target; it must list every live analysis. The current article is `/deep-dives/harness-problem/`, sourced at `site/deep-dives/harness-problem/index.html`. Add a new article to the index in the same release, then verify its canonical route, archive snapshot and sitemap entry. The current piece uses FCA and FSB evidence to explore remediation capacity, control design and correlated cyber disruption; do not treat it as a disposable promotional card.
- **Experience and design:** The compact-laptop homepage gate is a required 1366 × 768 check: hero calls-to-action must remain visible above the publication metrics with no overlap. Preserve the established navy, gold and ivory design system, token-only colours, squared corners, restrained shadows and non-looping motion. Responsive screenshot files under `site/qa/responsive/` are generated QA artefacts; do not stage incidental screenshot changes unless the task is specifically baseline update.
- **Cache and publication proof:** A local `file:///.../site-dist/` preview and a commit/push are not live proof. Build locally, then push to `main` and treat the green `Site release (Cloudflare)` workflow—with Pages/production exact-SHA checks and cache purge—as the sole production proof. Cache-busted inspection may show newer content before an ordinary crawler; investigate cache headers and workflow evidence rather than treating a local render as a release.
- **16 September edge audit:** Plain and cache-busted public requests to Home, Brief, Signals and Archive all returned the same current `46b5de1` release marker and the Edition 13 Home/Brief copy. Do not run a blind cache purge solely on a stale third-party crawl report; first probe `x-sgs-release` on the canonical routes. The audit did find an archive-hub navigation leak, which must be covered by the archive-template regression check.
- **16 September controls repair:** `scripts/verify_weekly_release_order.mjs` now compares the proposed Brief title and substantive Weekly Judgement paragraphs against the latest **three** archived editions, rather than only the preceding one. The Google Cloud status feed is governed as `service-status`: it needs a clearly material title, rejects maintenance, investigation and vague “some users may experience” notices, and deduplicates repeat incident labels. Do not weaken this filter to manufacture resilience volume.
- **Direct-source accounting:** `npm run signals:sources:validate` counts only direct primary feeds and topic coverage. The seven `reg_horizon_json` bridges are separately validated as the exact internal withheld route; they cannot inflate the 80-source floor or be mistaken for a public primary source. At the repair point the estate was 86 direct primary feeds, while the seven bridges remained explicitly withheld.
- **Operational assertions verified:** AI Signals is an exact 15-card current-edition product. The private Horizon workflow remains capped at 45 minutes and fails closed before SQLite/output writes below a 50% source-health floor. The public Worker remains `st-georges-strategy-not-found-route`, 1% head-sampled, and deployed only with `--keep-vars` to protect analytics configuration.
- **Distribution and Horizon integrity:** `distribution:pack` creates a private, versioned LinkedIn/newsletter/cover pack directly from `current-edition.json`; `distribution:verify` rejects any drift from the approved three-beat Judgement. Both editorial-prep and production-release workflows retain it as an artifact, never a public site asset. Horizon stability now counts three **distinct calendar-day** runs only—same-day retries replace one record rather than advancing the relaunch gate. Direct production Cloudflare deploy scripts refuse local execution and permit only the GitHub Actions release environment.
- **Release order:** Horizon private scan → Signals candidate review and approved shortlist → eight current Signal streams → AI Signals (15 cards) → current edition, Brief, Committee Questions, Home and Deep Dive → archive capture → data/build/link/responsive QA → guarded Cloudflare workflow. This order prevents a visually current site carrying old evidence.
...
- **16 September repair:** all seven bridge definitions were restored to `dashboard/data/signals-feed-registry.json` after an audit found that the runner still supported withheld status but the sources had been removed from the active registry. The validator now protects the complete bridge set and rejects incomplete or incorrectly labelled emitted bridge states. A focused non-persistent run confirmed all seven as `skipped` with `horizon-withheld`.
- **16 September release hardening:** the route-assignment script now fails if a retired site Worker remains attached outside the canonical route set; it will never silently leave a split-brain legacy route. The generated bundle asserts the exact three-file Worker asset exclusion list. Link transport exceptions are limited to individual reviewed URLs (no broad publisher/path wildcard), the legacy singular Committee Question must mirror `committeeQuestions[0]`, and every non-Horizon candidate feed must use a known canonical source owner within the capped 1–5-per-topic regime.
