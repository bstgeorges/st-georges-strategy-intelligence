...
1. The Sunday Reg Horizon scan creates a private review PR. Do not treat the scan workflow's successful PR creation as publication; review and merge it only as internal evidence.
2. `Signals candidates weekly refresh` collects and validates Signals candidates, then generates `dashboard/data/signals-promotion-review.generated.json` with `reviewStatus: pending`. The compact review begins with decision types (rule change, enforcement, active threat, outage and research), then shows source-owner concentration, UK/EMEA versus US balance, new-versus-continuing counts, and source-health actions. Candidate generation never publishes the site.
3. While Reg Horizon is withdrawn, Signals candidate generation is intentionally independent of the private scan. The bridge sources report `horizon-withheld` rather than pretending that a zero-yield Horizon is healthy. Weekly editorial prep may continue for the public Signals edition; it includes the private Horizon status for awareness only.
4. The editor reviews topic-readiness first, then any source-health `investigate` actions, concentration and duplication, before selecting only supported URLs and rationales in `dashboard/data/signals-promotion-shortlist.json`. Preserve the exact `candidateGeneratedAt` and set `reviewStatus` to `approved`. Resilience is a materiality gate, not a quota: do not promote a weak item merely to fill coverage; retain a verified anchor and record the gap when no candidate clears the editorial test. Every published Top 5 title must read as a concise editorial signal—the decision, control, dependency, or exposure it reveals—not as raw article, press-release, or incident-status copy. Retain the literal provider wording in `evidence.sourceTitle` and link directly to the source. arXiv is a supplementary research input: it may inform research discovery, but a quiet run does not create a primary-source coverage claim.
   - Source-owner and regional concentration warnings are an editorial review prompt, never a target. Resolve them through a materiality decision or a documented continuation; do not manufacture variety with a weaker signal.
5. The editor runs `Signals weekly publish draft`. It promotes the approved Signals rows, drafts the Brief, and opens a PR. Update `site/data/current-edition.json` with the new judgement and committee questions in the same reviewed PR.
   - Before approving the Judgement or LinkedIn copy, compare both with the previous three published editions. Record `judgement.editorialAngle` and `judgement.distinctFromPrevious` in `current-edition.json`. Reject a recurring generic action bundle; the new edition must state a specific decision type, failure mode and next move.
...
8. The `Site release (Cloudflare)` workflow runs the ordered-release gate. It refuses publication when candidate generation is newer than the approved shortlist, Signals are not current, the health report is absent, stale or unresolved, or the judgement is carried forward unchanged. The private Horizon has its own shadow-quality and relaunch gates.
   - The same release transaction generates a versioned distribution pack. Use its LinkedIn and newsletter copy verbatim for the three Judgement beats; review the cover and send a test email before distribution.
...
- Each key Top 5 display title reads as an editorial signal; exact source wording is retained only in its evidence record.
